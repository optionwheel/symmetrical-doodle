(() => {
  ...
  // Validiert die Textarea und synchronisiert daraus die Segmente des Rads.
  ...
  function drawWheel() {
    ...
    state.options.forEach((option, index) => {
      ...
      drawSegment(startAngle, endAngle, radius, index);
      drawSegmentLabel(option, startAngle + segmentAngle / 2, radius, segmentAngle);
    });
  ...
  function drawSegment(startAngle, endAngle, radius, index) {
  ...
  function drawSegmentLabel(option, angle, radius, segmentAngle) {
    ...
    ctx.shadowBlur = 8;
    ctx.fillText(label, 0, 0, radius * 0.44);
    ctx.restore();
  }
  ...
  // Animiert den Button-Spin kontrolliert und verhindert parallele Spins.
  function spinWheel() {
  ...
  function showResult(winner) {
    const commentPool = memeComments.concat(intenseMemeComments);
  ...
  // Kleine Ergebnis-Easter-Eggs für bestimmte Meme-Optionen.
  ...
  // Meme Mode lässt kurze Sprüche über den Bildschirm fliegen, ohne dauerhaft DOM zu füllen.
  ...
  // Physik-Loop: Winkelgeschwindigkeit plus Reibung bis das Rad natürlich stoppt.
  ...
  // Bei manuellem Rig nicht kurz "snappen", sondern natürlich weiter ausrollen lassen.
