(() => {
  ...
  const elements = {
    ...
    settingsTarget: document.getElementById("settingsTarget"),
  ...
  function updateOptions({ persist = true } = {}) {
    ...
    const previousSettingsTarget = elements.settingsTarget.value;
    state.options = nextOptions;
    if (elements.statusMessage.textContent === "the wheel has no aura rn ...") {
      elements.statusMessage.textContent = "";
    }
    syncSettingsSelect(previousSettingsTarget);
    state.wheelDirty = true;
    drawWheel();
    if (persist) {
      elements.optionsInput.defaultValue = elements.optionsInput.value;
  ...
  function syncSettingsSelect(previousValue = "") {
    elements.settingsTarget.innerHTML = '<option value="">Random</option>';
    state.options.forEach((option) => {
      const choice = document.createElement("option");
      choice.value = option;
      choice.textContent = option;
      elements.settingsTarget.appendChild(choice);
    });
    if (state.options.includes(previousValue)) {
      elements.settingsTarget.value = previousValue;
    }
  }
  ...
  function pickWinnerIndex() {
    const settingsValue = elements.settingsTarget.value;
    const settingsIndex = state.options.indexOf(settingsValue);
    return settingsIndex >= 0 ? settingsIndex : Math.floor(Math.random() * state.options.length);
  }
  function spinWheel() {
  ...
  function getSettingsIndex() {
    return state.options.indexOf(elements.settingsTarget.value);
  }
  let settingsClickCount = 0;
  let settingsClickTimer = null;
  function handleSettingsTrigger() {
  ...
  function bindEvents() {
    ...
    elements.optionsTitleButton.addEventListener("click", handleSettingsTrigger);
    ...
    elements.settingsTarget.addEventListener("change", () => {
